...
func TestPullRequestCommentFilter(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "trigger alone rejects without matching comment",
			triggerComment: "/kelos pick-up",
			wantAllowed:    false,
		},
		{
			name:           "trigger alone allows with matching comment",
			triggerComment: "/kelos pick-up",
			comments: []githubComment{
		...
		{
			name:           "trigger in body allows PR",
			triggerComment: "/kelos pick-up",
			body:           "/kelos pick-up",
			wantAllowed:    true,
		},
		...
		{
			name:            "no matching comments rejects PR",
			triggerComment:  "/kelos pick-up",
			excludeComments: []string{"/kelos needs-input"},
			wantAllowed:     false,
		},
		{
			name:            "trigger in body with both filters allows PR",
			triggerComment:  "/kelos pick-up",
			excludeComments: []string{"/kelos needs-input"},
			body:            "/kelos pick-up",
			wantAllowed:     true,
		...
		{
			name:            "body with both trigger and exclude rejects PR",
			triggerComment:  "/kelos pick-up",
			excludeComments: []string{"/kelos needs-input"},
			body:            "/kelos pick-up\n/kelos needs-input",
			wantAllowed:     false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotAllowed, gotTriggerTime := s.passesCommentFilter(tt.body, tt.comments)
